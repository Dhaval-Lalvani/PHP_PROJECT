...
// Query to count playlists created by each user
$sql = "SELECT username, COUNT(id) AS total_playlists
        FROM playlist_songs
        GROUP BY username";
...
<head>
    ...
    <title>Playlists Created by Each User Report</title>
    <style>
        body {
...
<body>
    <h2>Playlists Created by Each User</h2>
    ...
    <table>
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                echo "<tr><td>" . htmlspecialchars($row['username']) . "</td><td>" . $row['total_playlists'] . "</td></tr>";
            }
        } else {
